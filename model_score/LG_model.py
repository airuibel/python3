...
num_features_dic = {"icekredit_huiyan_score":6,"baidu_score":6,"fm_final_score":5}
for col in num_features:
    print("{} is in processing".format(col))
    max_interval = num_features_dic[col]
    cutOff = ChiMerge(df, col, 'is_over', max_interval=max_interval, special_attribute=[-1], minBinPcnt=0)         # 进行卡方分箱 离散化
    print(cutOff)
...
x_train = df_data.drop(['is_over'],axis=1)
y_train = df_data.loc[:,'is_over']
...
joblib.dump(cls, 'model_LG_test.pkl')
...
predict_proba_all = cls.predict_proba(x_train)  # 概率
predict_proba_over = predict_proba_all[:,1]     # 逾期概率
print(predict_proba_over)

train_prob_y = cls.predict(x_train)
print(train_prob_y)
train_auc = metrics.roc_auc_score(y_train,predict_proba_over)       #训练集上的auc值
print(train_auc)

# 计算KS值
fpr,tpr,thresholds=metrics.roc_curve(np.array(y_train),predict_proba_over)
...
list_s = []   #违约概率/正常概率 >> (Score = A - B*log(odds))   正常概率/违约概率 >> (Score = A + B*log(odds))
for i in range(len(predict_proba_all)):
    list_s.append(predict_proba_all[i][1] / predict_proba_all[i][0])

for i in range(len(list_s)):
    list_s[i] = int(120 - 40 * math.log(list_s[i]))

list_ses = pd.Series(list_s)
df['score'] = list_ses
print(df.head(50))
