...
params = {
    ...
    # 'objective': 'binary:logistic',  # 二分类的逻辑回归问题，输出为概率  reg:linear
    # "num_class": 2,  # 类别数，与 multi:softmax 并用
    "eval_metric": "auc",  # multi:softprob  ，multi:softmax  使用 merror
    "gamma": 0.1,  # 在树的叶子节点上进行进一步分区所需的最小损失减少。值越大，算法就越保守。
    "max_depth": 9,  # 构建树的深度，越大越容易过拟合
    "lambda": 10,  # 控制模型复杂度的权重值的L2正则化项参数，参数越大，模型越不容易过拟合。
    "subsample": 1,  # 随机采样训练样本
    "colsample_bytree": 1,  # 生成树时进行的列采样
    "min_child_weight": 3,  # 孩子节点中最小的样本权重和。如果一个叶子节点的样本权重和小于min_child_weight则拆分过程结束。在线性回归模型中，这个参数是指建立每个节点所需要的最小样本数。该参数越大算法越保守
    "silent": 0,  # 设置成1则没有运行信息输出，最好是设置为0.
    "eta": 0.08,  # 如同学习率
    "seed": 0,
    # "scale_pos_weight": 3,  #  如果你关心的预测的ranking order（AUC)： – 通过scale_pos_weight来平衡正负类的权重 – 使用AUC进行评估,控制正负样本权重的平衡。一般对非平衡的很有用。一个很特殊的取值是：负样本个数/正样本个数。
...
print(ans)
xx_train = xgb.DMatrix(X_train)
ans_train = model.predict(xx_train)
print(ans_train)


# 计算训练集KS值
train_fpr, train_tpr, train_thresholds = metrics.roc_curve(np.array(y_train), ans_train)
print("train数据集的XGB_KS : ", max(train_tpr - train_fpr))

plt.title("KS:{}".format(max(train_tpr - train_fpr)))
plt.plot(train_tpr)
plt.plot(train_fpr)
plt.show()

ans_train2 = (ans_train >= 0.5) * 1
print(ans_train2)
cm_train = metrics.confusion_matrix(y_train, ans_train2)  # 混淆矩阵 可计算精准度
print(cm_train)
...
"""
y_verify_train = y_train.values
y_verify_train = pd.DataFrame(y_verify_train)

"""
将numpy.array 转为DataFrame
"""
ans2_train = ans_train.tolist()
ans2_train = pd.DataFrame(ans2_train)
df_data_train = pd.concat([y_verify_train, ans2_train], axis=1)
df_data_train.columns = ["y_verify", "ans2"]
...
def play_roc(fpr,tpr,auc):
    """
    :param fpr: FPR=FP/（TN+FP）
    :param tpr:TPR=TP/（TP+FN）
    :param auc:AUC
    :return:ROC曲线
    """
    plt.figure()
    # plt.subplot(111,axisbg='#7FFF00')
    plt.plot(
        fpr,
        tpr,
        'b',
    ...
    plt.plot([0, 1], [0, 1], '--', color=(0.6, 0.6, 0.6)) # 画出对角线
    plt.xlim([0,1])
    plt.ylim([0,1])
...
play_roc(fpr=fpr,tpr=tpr,auc=test_auc)
...
data_df_train = to_report(df_data_train,cutoff)
print(data_df_train.head(1000))

data_df_test = to_report(df_data, cutoff)
print(data_df_test.head(1000))

# data_df_test.to_csv("data_df_report_reg_linear_2.csv", sep=",", index=False, header=True)
# model.save_model('xgbt_001.model')
